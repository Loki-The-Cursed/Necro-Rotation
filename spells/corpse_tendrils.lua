...
local function logics()
    ...
    local min_hits_required = menu_elements_corpse_base.min_hits:get()
    
    if best_cast_hits < min_hits_required then
        return false
    end

    if cast_spell.target(tendrils_data.corpse, spell_id_corpse_tendrils, 2.0, false) then
        local current_time = get_time_since_inject()
        -- Rapid casting with 0.1s delay for spam capability
