...
on_render_menu (function ()
    ...
    if not menu.main_tree:push("Necromancer: Base v0.1c") then
        return;
    end;
...
on_update(function ()
    ...
    -- Priority 1: Essential survival/buffs
    ...
    -- Priority 2: Minion management (high priority when enemies are near)
    -- Raise skeleton now includes enemy proximity check, so it's safe to check early
    ...
    -- Priority 3: Utility spells that affect combat
    if spells.soulrift.logics() then
        cast_end_time = current_time + 0.5;
        return;
    end;
    ...
    -- Priority 4: Area damage and crowd control
    ...
    -- Priority 5: High damage single target spells
    if spells.decompose.logics(best_target, entity_list) then
        cast_end_time = current_time + 0.5;
        return;
    end;
    ...
    -- Priority 6: Control and utility spells
    ...
    -- Priority 7: Filler/basic attacks
...
console.print("Lua Plugin - Necromancer Base - Version 1.6 (with Active/Inactive spell menus)");